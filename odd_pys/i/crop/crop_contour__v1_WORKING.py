...
def process(filename, key):
	...
	mask = np.zeros_like(closing) # Create mask where white is what we want, black otherwise
	...
	out = np.zeros_like(closing) # Extract out the object and place into output image
	out[mask == 255] = closing[mask == 255]
	cv2.drawContours(closing, contours, -1 , (255, 255, 255), -1)
